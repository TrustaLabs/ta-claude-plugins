...
2. **智能推荐文档清单**
   ...
   - 只有选择"自定义"时才展开多选列表
   - 推荐配置规则：
     * 前端项目 → ARCHITECTURE.md, COMPONENTS.md, TESTING.md
     * Node.js后端 → ARCHITECTURE.md, API.md, DATABASE.md, TESTING.md
     * Python后端 → ARCHITECTURE.md, API.md, DATABASE.md, TESTING.md
     * Claude插件 → PLUGIN_STRUCTURE.md, COMMANDS.md, SKILLS.md
     * 数据分析 → DATA_SOURCES.md, ANALYSIS_WORKFLOW.md, TESTING.md
     * 通用项目 → ARCHITECTURE.md, TESTING.md
...
3. **自动判断复杂模块**
   ...
   - 自动决策规则：
     ...
     * 文件数 < 5 → 不生成专门文档
   - 输出通知而非询问：
     "检测到 [模块名] 模块较复杂（[文件数]个文件），将为其生成 [文档名].md"
   - 删除 AskUserQuestion 调用
...
1. **智能检测文件冲突**
   - 使用 Read 工具检测文件是否存在
   - 如果文件存在，检查是否包含生成标识：
     `<!-- Generated by /ta-init on YYYY-MM-DD -->`
   - 处理逻辑：
     * 首次运行（文件不存在） → 直接生成
     * 包含生成标识且内容未变 → 直接更新
     * 包含生成标识但内容已修改 → 询问处理方式
